...
export async function collectStats(
  ...
  const aggregate: Aggregate = { sessions: 0, messages: 0, days: new Set(), models: new Map() };
...
class StatsViewer {
  ...
  private sortBy: "cost" | "tokens" = "cost";
  ...
  private maxOffset(): number {
    const models = this.state.kind === "done" ? this.state.aggregate.models.size : 0;
  ...
  render(width: number): string[] {
    ...
    if (this.state.kind === "scanning") {
      ...
      container.addChild(new Spacer(1));
      container.addChild(
        new Text(border(`lines ${from}–${to} of ${models.length}`) + (arrows ? th.fg("dim", `  ${arrows}`) : ""), 1, 0),
      );
    }

    container.addChild(
      new Text(th.fg("dim", `↑↓/j/k scroll · PgUp/PgDn page · s sort by ${this.sortBy === "cost" ? "tokens" : "cost"} · r rescan · esc close`), 1, 0),
    );
    container.addChild(new DynamicBorder(border));
