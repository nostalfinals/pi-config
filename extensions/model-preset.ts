...
import { CustomEditor, DynamicBorder, getAgentDir } from "@earendil-works/pi-coding-agent";
import {
  Container,
  type EditorTheme,
  type KeyId,
...
const CONFIG_PATH = join(getAgentDir(), "model-preset.json");
const DEFAULT_SHORTCUT: KeyId = "alt+shift+m";
...
function displayModel(preset: ModelPreset): string {
  return `${preset.provider}/${preset.model}`;
}

function legacyAltShiftSequence(shortcut: KeyId): string | undefined {
...
class ModelPresetEditor extends CustomEditor {
  constructor(
    tui: TUI,
    theme: EditorTheme,
    keybindings: KeybindingsManager,
    private readonly legacySequence: string,
    private readonly onLegacyShortcut: () => void,
  ) {
    super(tui, theme, keybindings);
  }

  override handleInput(data: string): void {
    // Legacy terminals encode Alt+Shift+letter as Escape + uppercase letter.
    // Pi's normal shortcut matcher needs Kitty/modifyOtherKeys to distinguish it.
    if (data === this.legacySequence) {
      this.onLegacyShortcut();
      return;
    }
...
export default function modelPresetExtension(pi: ExtensionAPI): void {
  ...
  async function showSelector(ctx: ExtensionContext): Promise<void> {
    ...
    try {
      const selected = await ctx.ui.custom<string | null>((tui, theme, _keybindings, done) => {
        const items = config.presets.map((preset, index) => {
          const presetText = `${displayModel(preset)} · ${preset.thinkingLevel}`;
          return {
            value: String(index),
            label: isActive(preset, ctx)
        ...
        const activeIndex = config.presets.findIndex((preset) => isActive(preset, ctx));
      ...
      const preset = config.presets[Number(selected)];
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    if (ctx.mode !== "tui" || !legacySequence || ctx.ui.getEditorComponent()) return;

    ctx.ui.setEditorComponent((tui, theme, keybindings) => new ModelPresetEditor(
      tui,
      theme,
      keybindings,
      legacySequence,
