...
- Don't write tests for reversible, low-impact changes that just mirror the implementation, and don't add a test for a deleted feature that merely asserts it no longer exists.
- Don't test or assert the exact or partial wording of prompts, messages, logs, or other non-contractual human-readable text. These tests just make our test suite expand uncontrollably and may cause trouble for us when implementing future features, so test the underlying state or behavior instead. Asserting fixture values is fine, and wording-only changes must not break tests.
- Run tests appropriate to the change and complete required checks. Once they pass, don't broaden or repeat testing unless new changes, failures, or unresolved concerns justify it.
